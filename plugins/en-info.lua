...
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   Username = '----'
    ...
    local text = '♨️ First name: '..(result.first_name or '')..' '..(result.last_name or '')..'\n'
               ..'♨️ User name: '..Username..'\n'
               ..'🆔 ID: '..result.id..'\n\n'
	local hash = '♨️ Rank:'..extra.chat2..':variables'
	local value = redis:hget(hash, result.id)
    if not value then
	 if result.id == tonumber(Arian) then
	   text = text..'♨️ Rank: Executive Admin \n\n'
	  elseif is_sudo(result.id) then
	   text = text..'♨️ Rank: Sudo \n\n'
	  elseif is_owner(result.id, extra.chat2) then
	   text = text..'♨️ Rank: Owner \n\n'
	  elseif is_momod(result
	      .id, extra.chat2) then
	    text = text..'♨️ Rank: Admin \n\n'
      else
	    text = text..'♨️ Rank: Member \n\n'
	 end
   else
   text = text..'♨️ Rank: '..value..'\n\n'
  end
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'info' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local text = '♨️ First name: '..(msg.from.first_name or '----')..'\n'
   local text = text..'♨️ Last name: '..(msg.from.last_name or '----')..'\n'	
   local text = text..'♨️ Username: '..Username..'\n'
   local text = text..'📱 phone: '..(msg.from.phone or 'no')..'\n'	
   local text = text..'🆔 ID: '..msg.from.id..'\n'
   local text = text..'♨️ Group name: '..msg.to.title..'\n'
   ...
   local hash = 'rank:'..msg.to.id..':variables'
	if hash then
	 ...
	 text = text..'📨 Total messages: '..user_info_msgs..'\n\n'
    if msg.to.type == 'chat' then
	 text = text..'♨️ Gruop name: '..msg.to.title..'\n'
