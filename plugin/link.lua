...
local function export_chat_link_callback(extra, success, result)
  ...
  return send_large_msg(receiver,'الان ارسل \n !lnk')
end
...
function run(msg, matches)
  ...
  if data[tostring(msg.to.id)] then
    ...
    -- group link {get|set}
    if matches[1] == 'ln' then
      local chat = 'chat#id'..msg.to.id
      if matches[2] == 'k' then
        if data[tostring(msg.to.id)]['link'] then
          local about = get_description(msg, data)
          local link = data[tostring(msg.to.id)]['link']
          us = 'user#id'..msg.from.id
          send_msg( us, msg.to.print_name.."\n\n"..link, ok_cb, false)
          return 'تم ☑️ ارسال الرابط خاص ➰#ملاحظه اذا لم يصل الرابط ❌ قم بأرسال 10 رسايل ليتم فتح الحضر👥'
        else
