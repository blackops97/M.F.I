...
local msgs = 
{
    already_locked = " تم بلفعل تفعيل مضاد شرب الخمر, -- the message that sent when you try to lock the fuck and it's already locked
    Locked = "تم تفعيل مضاد شرب الخمر, -- the message that send when you lock the fuck

    already_unlocked = "تم الغاء تفعصيل مضاد شرب الخمر, -- the message that sent when you try to unlock the fuck and it's already unlocked
    UnLocked = "تم الغاء مضاد شرب الخمر ويمكنك شرب ماتشاء�🙌🏿", -- the message that send when you unlock the fuck
...
local function run(msg, matches)
    ...
    if ( is_realm(msg) and is_admin(msg) or is_sudo(msg) or is_momod(msg) ) then
        ...
        elseif ( matches[2] == XCommands.CheckCommand ) then
            ...
            EXString = data[tostring(msg.to.id)]["settings"]["Lockbira"]
