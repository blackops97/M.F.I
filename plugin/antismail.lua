...
local msgs = 
{
    already_locked = "تم بالفعل تفعيل مضاد السمايلات", -- the message that sent when you try to lock the fuck and it's already locked
    Locked = "تم تفعيل مضاد السمايلات", -- the message that send when you lock the fuck

    already_unlocked = "تم بالفعل ايقاف تفعيل مضاد الكلمات السمايلات", -- the message that sent when you try to unlock the fuck and it's already unlocked
    UnLocked = "تم الغاء تفعيل مضاد السمايلات ,يمكنك قول ماتشاء 😒🙌🏿", -- the message that send when you unlock the fuck
...
local function run(msg, matches)
    ...
    if ( is_realm(msg) and is_admin(msg) or is_sudo(msg) or is_momod(msg) ) then
        ...
        if ( matches[2] == XCommands.LockCommand ) then
            ...
            data[tostring(msg.to.id)]['settings']['LockSmail'] = "yes"
        ...
        elseif ( matches[2] == XCommands.CheckCommand ) then
            ...
            EXString = data[tostring(msg.to.id)]["settings"]["LockSmail"]
