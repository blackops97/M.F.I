...
local msgs = 
{
    already_locked = "تم بالفعل تفعيل مضاد الطائفية🔚☑️", -- the message that sent when you try to lock the fuck and it's already locked
    Locked = "تم تفعيل مضاد الطائفية🔚عراقنا واحد لايفرقنا احد☑️🔒", -- the message that send when you lock the fuck

    already_unlocked = "تم ايقاف منع الطائفية🔚🔓", -- the message that sent when you try to unlock the fuck and it's already unlocked
    UnLocked = "تم ايقاف منع الطائفية🔚🔓", -- the message that send when you unlock the fuck

    statue = { Locked2 = "The tafia see is locked here", UnLocked2 = "The tafia see is unlocked here" }
}
...
local function run(msg, matches)
    ...
    if ( is_realm(msg) and is_admin(msg) or is_sudo(msg) or is_momod(msg) ) then
        ...
        if ( matches[2] == XCommands.LockCommand ) then
            ...
            data[tostring(msg.to.id)]['settings']['Locktafia'] = "yes"
        ...
        elseif ( matches[2] == XCommands.CheckCommand ) then
            ...
            EXString = data[tostring(msg.to.id)]["settings"]["Locktafia"]
